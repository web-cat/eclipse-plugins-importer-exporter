...
import org.webcat.eclipse.deveventtracker.EclipseSensorConstants;
...
public class JavaStructureChangeDetector implements IElementChangedListener {
		...
		// Addition, deletion, renaming activity.
			...
			if (deletions.isEmpty()) {
				...
				if (fromDelta.getElement().getParent()
					...
					javaFile = fromDelta.getElement().getResource()
							.getLocation();
					processMoveRefactor(javaFile, fromDelta.getElement(),
							fromDelta.getElement().getParent(), toDelta
	...
	private void processUnary(IPath javaFile, String op, IJavaElementDelta delta) {
		...
		if (!EclipseSensorConstants.JAVA.equals(classFileName
				.getFileExtension())) {
			return;
		}
		...
		if (name != null && !"".equals(name)) {
			...
			URI projectURI = element.getResource().getProject()
						.getLocationURI();
			this.sensor.addDevEvent("Edit", projectURI, resource, devEventPMap,
					op + " " + name);
		}
	}
	...
	private void processRenameRefactor(IPath javaFile,
		...
		if (fromName != null && !"".equals(fromName) && toName != null
			...
			msgBuf.append("Refactor : Rename#").append(typeName).append('#')
			...
			URI resource = fromDelta.getElement().getResource()
					.getLocationURI();
			URI projectURI = fromDelta.getElement().getResource().getProject().getLocationURI();
			this.sensor.addDevEvent("Edit", projectURI, resource, devEventPMap,
					msgBuf.toString());
		}
	}
	...
	private void processMoveRefactor(IPath javaFile, IJavaElement element,
		...
		if (fromName != null && !"".equals(fromName) && toName != null
				&& !"".equals(toName)) {
			StringBuffer msgBuf = new StringBuffer();
			msgBuf.append("Refactor : Move#").append(typeName).append('#')
					.append(name).append('#').append(fromName).append(" -> ")
					.append(toName);
			...
			URI resource = element.getResource().getLocationURI();
			URI projectURI = element.getResource().getProject().getLocationURI();
			this.sensor.addDevEvent("Edit", projectURI, resource, devEventRenameMap,
					msgBuf.toString());
		}
	}
