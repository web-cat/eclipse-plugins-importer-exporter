...
public class BuildErrorSensor {
	...
	public void findBuildProblem(IResourceDelta delta) {
		...
		for (int i = 0; i < markerDeltas.length; i++) {
			...
			Map<String, String> keyValueMap = processPossibleBuildErrors(
					fileResource, markerDelta);
			if (!keyValueMap.isEmpty()) {
				...
				if (!messagePool.contains(data)) {
					...
					this.sensor.addDevEvent("Build", projectURI, fileResource,
