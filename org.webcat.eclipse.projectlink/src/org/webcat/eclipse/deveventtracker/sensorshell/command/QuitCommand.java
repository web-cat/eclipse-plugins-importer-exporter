...
/**
 * Implements the Quit command, which sends any buffered data and closes the
 * loggers.
 * 
...
public class QuitCommand extends Command {
	 ...
	 * @param sensorDataCommand
	 *            The SensorDataCommand.
	 * @param autoSendCommand
	 *            The AutoSendCommand.
	 */
	...
	public void quit() throws SensorShellException {
		...
		try {
			...
			// Note that we had an exception thrown, now proceed to finally
			// block.
			exception = e;
			...
			if (exception != null) {
				this.shell.println("Error sending data during final quit: "
						+ exception);
			}
			this.shell.println("Quitting SensorShell started at: "
					+ this.shell.getStartTime());
			this.shell.println("Total sensor data instances sent: "
					+ this.shell.getTotalSent());
		}
		// If we had an autosend exception, and there's data left, we throw it
		// again.
		if ((this.autoSendCommand.getException() != null)
				&& (this.sensorDataCommand.hasUnsentData())) {
			throw this.autoSendCommand.getException();
		}
		// If we had a quit() exception, and there's data left, we throw it
		// again.
