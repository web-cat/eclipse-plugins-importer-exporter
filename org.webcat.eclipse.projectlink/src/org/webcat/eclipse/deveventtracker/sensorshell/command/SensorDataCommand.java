...
public class SensorDataCommand extends Command {
	...
	/**
	 * Given a Map containing key-value pairs corresponding to SensorData fields
	 * and properties, constructs a SensorData instance and stores it for
	 * subsequent sending to the SensorBase.
	 * 
	 * @param keyValMap
	...
	/**
	 * Adds the SensorData instance, invoking send if the max buffer size has
	 * been exceeded.
	 * 
	 * @param data
	...
	public void add(SensorData data) throws SensorShellException {
		...
		if (sensorDatas.getSensorData().size() > properties
			...
			this.shell.println("Invoking send(); buffer size > "
					+ properties.getAutoSendMaxBuffer());
			try {
				this.send();
	 ...
	 * From the sensor side, StateChange can be implemented by creating a timer
	 * process that wakes up at regular intervals and gets the file (resource)
	 * that the current active buffer is attached to, plus a "checksum"
	 * (typically the size of the resource in characters or bytes or whatever).
	 * Then, the timer process simply creates the appropriate keyValMap as if
	 * for an 'add' event, and provides it to this method along with the buffer
	 * size. This method will check the passed buffer size and the Resource
	 * field against the values for these two fields passed in the last call of
	 * this method, and if either one has changed, then the "add" method is
	 * called with the keyValMap.
	 * 
	 * @param resourceCheckSum
	 *            Indicates the state of the resource, typically via its size.
	 * @param keyValMap
	 *            The map of key-value pairs.
	 * @throws Exception
	 *             If problems occur while invoking the 'add' command.
	 */
	public void statechange(long resourceCheckSum, Map<String, String> keyValMap)
		...
		String resource = (keyValMap.get("URI") == null) ? "" : keyValMap
				.get("URI");
		// Do an add if the resource or buffer size has changed.
		if (!this.lastStateChangeResource.equals(resource)) { // NOPMD
			this.shell.println("Invoking add: Resource has changed to: "
					+ resource);
			this.add(keyValMap);
		} else if (this.lastStateChangeResourceCheckSum != resourceCheckSum) { // NOPMD
			this.shell.println("Invoking add: CheckSum has changed to: "
					+ resourceCheckSum);
			this.add(keyValMap);
		} else {
			this.shell.println("No change in resource: " + resource
	...
	/**
	 * Returns the value associated with key in keyValMap, or the default if the
	 * key is not present.
	 * 
	 * @param keyValMap
	 *            The map
	 ...
	 *            The key
	 * @param defaultValue
	 *            The value to return if the key has no mapping.
	 * @return The value to be used.
	 */
	private String getMap(Map<String, String> keyValMap, String key,
			String defaultValue) {
		return (keyValMap.get(key) == null) ? defaultValue : keyValMap.get(key);
	}

	/**
	 * Returns true if the passed key is not one of the standard Sensor Data
	 * fields including "Timestamp", "Runtime", "Tool", "Resource", "Owner", or
	 * "SensorDataType".
	 * 
	 * @param key
	 *            The key.
	 * @return True if the passed key indicates a property, not a standard
	 *         sensor data field.
	 */
	...
	/**
	 * Returns true if this SensorDataCommand instance has remaining unsent
	 * data.
	 * 
	 * @return True if there is data remaining to be sent.
	 */
	public boolean hasUnsentData() {
		return ((this.sensorDatas.getSensorData() != null) && !this.sensorDatas
				.getSensorData().isEmpty());
	}
